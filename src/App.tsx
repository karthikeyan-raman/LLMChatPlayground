...
import { Chat, ChatMessage, Attachment, LLMModel as Model, FileType } from './types';
...
const PlaygroundLogo = (props: SvgIconProps) => (
...
const AnthropicIcon = (props: SvgIconProps) => (
...
const isUserMessage = (message: ChatMessage): boolean => message.role === 'user';

// Define available models as a constant outside of component
const AVAILABLE_MODELS: Model[] = [
  ...
  },
  {
    ...
    defaultTokens: 4096,
    defaultTemperature: 0.7,
    capabilities: ['Speed', 'Code', 'Summarization']
  },
  {
    id: 'amazon-nova-pro',
    ...
    defaultTokens: 4096,
    defaultTemperature: 0.7,
    capabilities: ['Analysis', 'Reasoning', 'Knowledge']
  },
  {
    id: 'gpt-4-turbo',
    name: 'GPT-4 Turbo',
    ...
    defaultTokens: 4096,
    defaultTemperature: 0.7,
    capabilities: ['Reasoning', 'Knowledge', 'Vision']
  },
  {
    id: 'gpt-4o',
    name: 'GPT-4o',
    ...
    defaultTokens: 4096,
    defaultTemperature: 0.7,
    capabilities: ['Speed', 'Reasoning', 'Code']
  },
  {
    id: 'o1-mini',
    name: 'o1-mini',
    ...
    defaultTokens: 2048,
    defaultTemperature: 0.7,
    capabilities: ['Speed', 'Efficiency', 'Chat']
  }
];

const App: React.FC = () => {
  // Use the available models defined above
  const [models] = useState<Model[]>(AVAILABLE_MODELS);
  
  // Initialize with a timestamp to ensure consistency
  const initialTimestamp = useMemo(() => Date.now(), []);
  
  // Chat data - adapt to work with existing code while matching the Chat type
  ...
  // Get current chat - memoized to avoid recalculation on every render
  const currentChat = useMemo(() => {
    return chats.find(chat => chat.id === currentChatId) || chats[0];
  }, [chats, currentChatId]);

  // Get current model - memoized to avoid recalculation
  ...
  const handleModelChange = useCallback((event: SelectChangeEvent) => {
    ...
    if (model) {
      // Use safe defaults if the model properties are missing
      setTemperature(model.defaultTemperature || 0.7);
      setMaxTokens(model.defaultTokens || 4096);
        ...
        setChats(prevChats => 
          prevChats.map(chat => 
            chat.id === currentChatId 
              ? { ...chat, modelId: modelId, updatedAt: Date.now() } 
              : chat
          )
        );
      }
    }
  }, [models, currentChatId]);
  ...
  const handleResetSettings = useCallback(() => {
    ...
    // Use safe defaults if properties are undefined
    setTemperature(model.defaultTemperature || 0.7);
    setMaxTokens(model.defaultTokens || 4096);
  }, [getCurrentModel]);
  ...
  const handleDeleteChat = useCallback((chatId: string) => {
    setChats(prevChats => {
      ...
      if (currentChatId === chatId && newChats.length > 0) {
        // We need to use setTimeout to avoid a React state update conflict
        setTimeout(() => {
  ...
  const processFiles = useCallback((files: File[]) => {
    ...
    const newAttachments: Attachment[] = Array.from(files).map(file => {
      ...
      // Create attachment with a unique ID
  ...
  // Remove an attachment and revoke its URL to prevent memory leaks
  const handleRemoveAttachment = useCallback((id: string) => {
    setAttachments(prevAttachments => {
      // Find the attachment to remove
      const attachmentToRemove = prevAttachments.find(attachment => attachment.id === id);
      if (attachmentToRemove?.url) {
        // Revoke the object URL to prevent memory leaks
        URL.revokeObjectURL(attachmentToRemove.url);
      }
      // Filter out the attachment
      return prevAttachments.filter(attachment => attachment.id !== id);
    });
  }, []);
  ...
  const handleSendMessage = useCallback(() => {
    ...
    // Add user message with ChatMessage format but adapt for backward compatibility
    ...
    setShowAttach(false);
    setIsLoading(true);
  }, [inputMessage, attachments, currentChatId, chats]);
    
    // The simulated response should be moved inside the handleSendMessage function
    // to avoid unexpected behavior with the useCallback hook
  
  // Make real API calls to chat models
  const sendMessageToAPI = useCallback(async () => {
    ...
    // We need only the last few messages for context, not the entire history
    const lastMessages = chat.messages.slice(-10); // Get the last 10 messages for context
    const messages = lastMessages.map(msg => ({
      role: msg.role,
      content: msg.content
    }));
    
    // Generate a timestamp and message ID for the response
    const timestamp = Date.now();
    const msgId = timestamp.toString();
    
    try {
      // If streaming is enabled, add an empty placeholder message first
      if (streamResponse) {
        setChats(prevChats => 
          prevChats.map(chat => {
            if (chat.id === currentChatId) {
      ...
      // For now, let's use a simulated response for debugging
      ...
      // We've disabled the actual API call and are using a simulated response now
      // This ensures the UI works correctly while we debug the API integration
      ...
      const response = await sendChatRequest({
        ...
        maxTokens,
        temperature,
        attachments: messages[messages.length - 1].attachments?.map(a => ({
          name: a.name,
          type: a.type,
          content: a.content
        }))
      ...
      */
      ...
      // For streaming mode, update the existing message
      if (streamResponse) {
        setChats(prevChats => 
          prevChats.map(chat => {
            if (chat.id === currentChatId) {
        ...
        // For non-streaming mode, add the full response
      ...
      const errorMessage: ChatMessage = {
        id: msgId,
        content: `Error: Failed to get a response from ${getCurrentModel().name}. Please try again.`,
        role: 'assistant',
        timestamp,
        modelId: selectedModel
      };
          ...
          >
            {currentChat.messages.map((message) => (
              <Box
                key={message.id}
                sx={{
                  alignSelf: isUserMessage(message) ? 'flex-end' : 'flex-start',
                  backgroundColor: isUserMessage(message) ? 'primary.dark' : 'background.default',
                  p: 2,
                  borderRadius: 2,
                  mb: 2,
                  maxWidth: '70%',
